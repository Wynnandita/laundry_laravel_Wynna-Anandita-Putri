...
export default {
    ...
    this.axios.get(`http://127.0.0.1:8000/api/transaksi/${this.id_transaksi}`, { headers : { 'Authorization' : 'Bearer ' + this.$store.state.token} })
             .then(res => {
                 this.transaksi = res.data
             })
             .catch(err => console.log(err))
    this.axios.get(`http://127.0.0.1:8000/api/transaksi/detail/${this.id_transaksi}`,
                  { headers : { 'Authorization' : 'Bearer ' + this.$store.state.token} })
              .then((res) => {
                  this.detail = res.data
              }) 
              .catch(err => console.log(err))      
    this.axios.get(`http://127.0.0.1:8000/api/transaksi/total/${this.id_transaksi}`,
                  { headers : { 'Authorization' : 'Bearer ' + this.$store.state.token} })
              .then(res => {
    ...
    ubahStatus() {
        ...
        this.axios.post(`http://127.0.0.1:8000/api/transaksi/status/${this.id_transaksi}`,
                         this.transaksi,
                         { headers : { 'Authorization' : `Bearer ` + this.$store.state.token} })
            .then(  () => {
                this.$router.go();
    ...
    bayar() {
    this.axios.post(`http://127.0.0.1:8000/api/transaksi/bayar/${this.id_transaksi}`,
                         this.transaksi,
                         { headers : { 'Authorization' : `Bearer ` + this.$store.state.token} })
            .then(  () => {
                this.$router.go();
